...
## 二、 内存管理与大小计算：DMA & Zero-Copy 底层原理
...
## 三、 高通通用内存压缩技术：UBWC (Universal Bandwidth Compression)
...
在移动端 Camera 系统中，Sensor 驱动是软硬件交互的第一站。理解 Sensor 配置，本质上就是弄清楚“控制信号怎么发过去（CCI/I2C）”**、**“图像数据怎么传回来（MIPI CSI-2）”**以及**“关键参数怎么实时控制（曝光/增益/寄存器）”。

---

### 一、 控制通道：CCI (Camera Control Interface) 与 I2C
...
### 三、 Sensor 寄存器配置与 Mode 切换

Sensor 驱动的核心工作之一，就是维护一组结构化的**寄存器配置表（Register Arrays）**。

#### 1. 初始化与模式寄存器配置（Init & Mode Settings）
